...
export class AwsS3 {
    ...
    async upload (dataUpload: Buffer | string | Readable, folder: string, extension: string, contentType: string, s3BucketName: string, keyToSaveInBucket = ''): Promise<IUploadAwsS3> {
        ...
        try {
            const result = await this.connection.upload({
                ...
                Key: `${folder}/${keyToSaveInBucket || uuid()}.${extension}`,
                ACL: 'public-read'
            }).promise()
            return result
        } catch (error) {
    ...
    async getSignedUrlPromise (folder: string, extension: string, s3BucketName: string, contentType: string): Promise<string> {
        try {
            const result = await this.connection.getSignedUrlPromise('putObject', {
                Bucket: s3BucketName,
